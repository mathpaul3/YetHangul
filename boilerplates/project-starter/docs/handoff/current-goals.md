...
6. Atomic Queue가 소진됐는데 spec이나 proof gap이 남아 있으면, 남은 gap을 기준으로 새 smoke/proof task 묶음을 다시 만든다.
...
5. 프로젝트 진행 중 범용적으로 재사용할 수 있는 개념, 운영 규칙, proof 패턴이 생기면 boilerplate/process 문서도 함께 갱신한다.
6. 구현 마무리 단계의 nav / footer / legal / analytics / branding은 `service shell` 묶음으로 계획하고 처리한다.
