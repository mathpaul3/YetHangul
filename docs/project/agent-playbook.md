...
- 버전은 커밋마다 올리지 않고, 릴리스 가능한 사용자 단위가 모였을 때만 올린다.
- 릴리스 커밋에는 반드시 같은 버전의 `vX.Y.Z` git tag를 붙인다.
- 배포 파이프라인은 가능하면 브랜치 push보다 `v...` tag push를 기준으로 trigger한다.
- pre-1.0 단계에서는 기본적으로 `0.MINOR.PATCH` 운영을 권장한다.
- commit 전 권장 기준:
  - 빌드 통과
  - 핵심 테스트 통과
  - handoff 문서 갱신
...
- 작은 화면에서 긴 keycap label이 레이아웃을 깨면 아이콘으로 치환하되, 접근성 라벨과 테스트 식별자는 별도로 유지한다.
