...
- 멀티 Agent 구조라면 기본적으로 Coordinator만 commit한다.
...
- 같은 고수준 task가 두 라운드 이상 반복되면 더 작은 sub-task id로 다시 분해한다.
- 한 cycle이 끝나면 개발 과정 자체에 대한 review를 진행하고, reusable insight는 boilerplate에도 반영한다.
...
- Operating Rule:
  ...
  - commit은 원칙적으로 Coordinator만 수행한다.
...
## 7. Cyber Security / AppSec Agent

- Profile:
  - 애플리케이션 보안, 비밀정보 관리, 브라우저 보안 정책, 공급망 위험, 인증/인가 경계에 강한 보안 엔지니어
- Goal:
  - 프로젝트가 기능을 늘려가도 기본 보안 위생과 위협 모델이 무너지지 않게 유지
- Best For:
  - secrets vs public config 구분
  - XSS / injection / unsafe HTML 검토
  - CSP / headers / third-party script review
  - dependency / supply-chain risk review
  - auth / session / permission boundary review
  - data exposure / logging / privacy review
