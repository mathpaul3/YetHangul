...
- `R25-3c` `Landed`
  - desktop에서는 native composition + mouse selection + on-screen replacement, mobile에서는 native composition + touch selection + on-screen delete가 같은 editor mutation surface를 사용하는 e2e proof를 고정했다.
